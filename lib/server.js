            ...
            for(i = 0, l = as.length; i < l; i++) {
                ...
                if ( false
                       ...
                       if(!item.handler && item.file) {
                           request._proxy_handler = (function(file, dir) {
                               return function() {
                                   if(!fs.existsSync(file)) {
                                       file = path.resolve(dir + path.sep + file);
                                   }
                                   edp.log.info(edp.chalk.yellow('PROXY') + ' ' + edp.chalk.green(url) + ' to ' + edp.chalk.green(file));
...
function render(context, root, gConf) {
    ...
    var renderConf = indexer.getRenderConf(queryObj.wd, queryObj.dsp);

    // 编译page.tpl
    for(var tplName in renderConf.left) {
        build.merge(
            path.dirname(renderConf.left[tplName].tpl),
            tplName,
            {}
        );
    }
    for(tplName in renderConf.right) {
        build.merge(
            path.dirname(renderConf.right[tplName].tpl),
            tplName,
            {}
        );
    }
