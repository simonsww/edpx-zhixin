...
function getRenderConf(query, platform, gConf) {
    ...
    if(util.isWisePlatform(platform)) {
        ...
        var wisePlatform = platform instanceof Array ? platform : [platform];

        if(gConf.base) {
            var baseObj = gConf.base;
            wisePlatform.forEach(function(item) {
                var absBase = util.getAbsPath(baseObj[item]);
                if(!absBase) {
                    absBase = path.resolve( __dirname, '../env/aladdin/base/' + item + '.tpl');
                }
                result.base.tpl.push(absBase);
            });
        }
