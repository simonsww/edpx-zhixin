...
function rewriteTplContent(tplContent, platform) {
    var content = String(tplContent)
        .replace(
            /\{%extends\s+"aladdin\/base\/`\$pageData\.tpl`\.(?:`\$pageData\.tplType`|tpl)"%\}/, 
            function($0, $1, $2) {
                return '{%extends "aladdin/base/' + platform + '.tpl"%}';
            }
        );
    return content;
}
...
exports.render = function(php, config) {
    ...
    if(config.base.tpl && config.base.tpl.forEach) {
        var wiseBase = 'aladdin' + sep + 'base';
        var filename;
        var baseTplDir = tplDir + sep + wiseBase;
        //如果没有基础目录，则创建
        if(!fs.existsSync(baseTplDir)) {
            wrench.mkdirSyncRecursive(baseTplDir, 0777);
        }

        config.base.tpl.forEach(function(tpl) {
            filename = path.basename(tpl);
            util.copyFile(tpl,  baseTplDir + sep + filename);
        });
    }
