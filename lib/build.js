...
function buildAll(root, opts) {
    ...
    var projectRoot = util.findFile(root, './page');
    ...
    if (fs.existsSync(projectRoot)) {
        ...
        edp.log.warn('>> No `page` folder in ' + root);
    }
}
...
function buildUnit(root, tplName, opts) {
    root = root || process.cwd();
    var pagePath = util.findFile(root, 'page/' + tplName);

    if (fs.existsSync(pagePath)) {

        var tplPath = path.resolve(pagePath + '/page', tplName);
        buildUnitByPath(tplPath, tplName, opts);
    }
    else {
