...
function buildAll(opts) {
    ...
    var projectRoot = util.findFile(process.cwd(), './page');
    
    if (fs.existsSync(projectRoot)) {
        var pagePath = path.resolve(projectRoot, './page');
        fs.readdirSync(pagePath).forEach(
            function (item) {
                var unitPath = path.resolve(pagePath, item);
        ...
        edp.log.warn('>> No `page` folder in ' + process.cwd());
    }
}
 ...
 * @param  {Object} opts    参数
 */
function buildUnit(tplName, opts) {
    ...
    if (fs.existsSync(pagePath)) {
        ...
        if (fs.existsSync(lessPath)) {
            lessCompiler(lessPath, opts, function () {
                ...
                edp.log.info('>> `' + tplName + '` ' + opts.info + ' success.');
            });
        }
        else {
