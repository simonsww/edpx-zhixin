...
function jsUglify(filePath) {
    ...
    try {
        ...
        var final_code = UglifyJS.minify(filePath,
            {
...
function buildUtpl(filePath, fileName, opts) {
    ...
    if(opts.r) {
        try {
            var UglifyJS = require("uglify-js");
            var final_code = UglifyJS.minify(compiled,
                {
                    fromString: true,
                    output: {max_line_len: 512}
                }
            ).code;
            codeMap[fileName] = final_code;
        }
        catch (ex) {
            edp.log.error('>> errors in ' + filePath);
            edp.log.error('>> ' + ex.message
...
function buildAll(root, opts) {
    ...
    for (var i = 0, l = dirs.length; i < l; i ++) {
        ...
        if (fs.statSync(dirPath).isDirectory()) {
            if (item == 'page') {
                fs.readdirSync(path.resolve(root, './page')).forEach(
                    function (tplname) {
