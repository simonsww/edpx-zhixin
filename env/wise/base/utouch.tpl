...
{%/block%}
    ...
    {%if $tplData.url||$tplData.titleurl%}<a class="result_title" href="{%if $tplData.url%}{%fe_fn_get_url url={%$tplData.url%} type='title'%}{%elseif $tplData.titleurl%}{%fe_fn_get_url url={%$tplData.titleurl%} type='title'%}{%/if%}">{%else%}<span class="result_title" style="text-decoration:none;">{%/if%}{%$tplData.title|wordtranscode:"UTF-8":"GBK"%}{%if $tplData.url||$tplData.titleurl%}</a>{%else%}</span>{%/if%}
    {%/if%}
    {%/block%}
    ...
    <span class="site">{%if $tplData.site%}{%$tplData.site%}{%elseif $tplData.showurl%}{%$tplData.showurl|regex_replace:'/\/.*$/':''%}{%/if%}</span>&#160;
