...
{%if $css%}
{%include fe_var_temp_ui_sname=$sname file="search/searchaladdin/_include/css/`$css`/`$pageData.tpl`.`$pageData.tplType`" inline%}
{%/if%}
{%/function%}
{%function name='fe_fn_get_ui' ui='' data=''%}
{%if $ui%}
{%include fe_var_temp_ui_data=$data file="search/searchaladdin/_include/ui/`$ui`/`$pageData.tpl`.`$pageData.tplType`" inline%}
{%/if%}
{%/function%}
{%block name="wrapper_prefix"%}
<div class="result" srcid="{%$alaData.resourceid%}" tpl="{%basename($smarty.current_dir)%}" data-log="{%$fe_var_log_data%}" order="{%$alaData.iteration%}">
{%/block%}
	{%*样式区块*%}
