{%function name="fis_widget_baidu_se_sp_section2"%}{%if $res.dispData.strategy.tempName=="naturalresult"%}<li class="bds-list" id="{%$resIndex%}" tpl="se_as_default_section2"><div class="bds-item-box"><a data-click="{    'F':'{%$res.strategyStr[0]%}',    'F1':'{%$res.strategyStr[1]%}',    'F2':'{%$res.strategyStr[2]%}',    'F3':'{%$res.strategyStr[3]%}',    'T':'{%$serverTime%}',        'url':this.href,        'y':'{%$res.identifyStr%}'        }" href="{%enc_url url=$res.dispData.resultData.tplData.url%}" target="_blank"><h3>{%$res.dispData.resultData.tplData.title|limitlen:63|highlight:4%}</h3><p> {%$res.dispData.resultData.tplData.content|highlight:4%} </p></a> <p class="showurl"> <cite> {%$res.dispData.resultData.tplData.showurl%} </cite><time>{%$res.dispData.resultData.tplData.date%}</time> - <a data-nolog="" href="{%$cache_domain%}/c?{%$res.dispData.resultData.tplData.capture%}&user=baidu&fm=sc&query={%$queryUbs%}&qid={%$queryId%}&p1={%$resIndex%}" target="_blank"> 百度快照</a></p></div></li>{%else%}{%if isset($res.dispData.resultData.extData)%}{%$res.dispData.resultData.extData.feData=[
		'showAladdinOuter' => 1,
		'newGrid' => 1,
