...
{%function fe_fn_c_title_suffix title='' url=''%}</h3>{%/function%} 

{%function fe_fn_c_showurl_tools title='' url=''%}{%/function%}

{%function fe_fn_c_showurl showurl='' date='' showlamp=''%}{%if $showurl===''%}{%$showurl=$tplData.showurl%}{%/if%}{%if $date===''%}{%$date=$tplData.date%}{%/if%}{%if $showlamp===''%}{%$showlamp=$tplData.showlamp%}{%/if%}<p class="c-showurl"><cite>{%$showurl|escape:'html'%}</cite> {%if $date%} <time>{%$date|escape:'html'%}</time> {%/if%} {%if $showlamp%} - <a target="_blank" href="http://open.baidu.com/" class="aladdin-LAMP"></a>{%/if%}</p>
{%/function%} 
