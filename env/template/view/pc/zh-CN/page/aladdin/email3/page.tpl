{%extends 'c_base.tpl'%} {%block name='data_modifier'%}{%$tplData.showgw=true%}{%$extData.feData.hasBorder=true%}{%/block%}{%block name='content'%}<div> <script >var op_email3_authCodeSrc = '{%if $tplData.authenticode[0]['name']%}{%$tplData.authenticode[0]['src']|escape:'html'%}{%/if%}'; </script> {%if $tplData.content1%}<div style="font-size:13px;marign-top:5px;">{%$tplData.content1|highlight:0%}</div>{%/if%} <form target="_blank" autocomplete="off" action="{%$tplData.action|escape:'html'%}" method="post"
