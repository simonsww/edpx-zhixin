...
{%block name='data_modifier'%}
    {%$escaped_card_name='ecl-edu-examine-course'%}
...
.ecl-edu-examine-course-title{margin-top:0}.ecl-edu-examine-course-container{line-height:1.7692307692;position:relative}.ecl-edu-examine-course-container h4,.ecl-edu-examine-course-container h5{margin:0;font-weight:normal}.ecl-edu-examine-course-no-data{padding:2.5em 1em;text-align:center}.zx-tip-layer{display:block}.c-tip-con .c-tip-cer{width:354px;color:#666;margin:0 10px 1px 10px}.opui-honourCard{font-size:12px;color:#666;line-height:1.5em;width:354px}.opui-honourCard-score{width:60px;padding:4px 2px 4px 2px;text-align:center;float:left}
.opui-honourCard-score em{display:block;font-size:37px;line-height:37px;color:#8e8d8d;font-family:arial;font-style:normal}.opui-honourCard-info{background:#f5f5f5;overflow:hidden;zoom:1}.opui-honourCard-title a{*font-family:Microsoft YaHei}.opui-honourCard-title a i{position:relative;left:-65px;*left:-68px}.opui-honourCard ol{list-style:none;margin:0;padding:0;float:left;_padding-top:5px}.opui-honourCard li{height:20px;line-height:20px;padding-left:20px;_vertical-align:bottom}.opui-honourCard li i{vertical-align:middle;_vertical-align:bottom}
.opui-honourCard-selected{color:#2b2a2a;font-weight:bold;background:url(http://www.baidu.com/cache/aladdin/ui/honourCard/img/honourCard.png) no-repeat 3px 5px;*background-position:3px 2px}.ecl-edu-examine-course-container{border-top:1px solid #f3f3f3}
</style>



{%strip%}
<table class="{%$escaped_card_name%}-container c-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <span class="c-gap-right-small{%if NOT $item_info@first%} c-gap-left-small{%/if%}">
            ...
            {%if $item_info.vcard and ($item_info.vcard != "[]")%}
                {%$item_vcard = json_decode($item_info.vcard, true)%}

                {%if isset($item_vcard.value2)%}
                <a class="c-icon c-icon-v c-icon-v{%$item_vcard.value2%} c-gap-icon-left-small" data-tooltips="bl"
                    data-renzheng='{%$item_info.vcard|escape:'html'%}'></a>
                {%/if%}
            {%/if%}
