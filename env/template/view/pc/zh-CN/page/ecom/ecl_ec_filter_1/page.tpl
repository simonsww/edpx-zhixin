        ...
        <div class="ecl-weigou-header">
            <div class="ecl-weigou-header-inner">
                <div class="ecl-weigou-header-left">
                    {%assign var="weigou_title" value=($extData.OriginQuery|cat:"（共"|cat:$tplData.totalCount|cat:"款）最新"|cat:$extData.OriginQuery|cat:" 最热"|cat:$extData.OriginQuery|cat:"_百度微购") %}
                    <a href="http://weigou.baidu.com/search?q={%$extData.OriginQuery|escape:'url'%}" title="{%$weigou_title|escape:'html'%}" target="_blank">{%$weigou_title|escape:'html'|limitlen:90|highlight:0%}</a>
                </div>
                    ...
                    {%if $tplData.totalCount == 2%}
                        ...
                        {%foreach $tplData.goods as $item%}
                            ...
                            <div class="ecl-weigou-product" data-id="{%$item.id%}">
                                ...
                                {%if $tplData.need_transfer%}<a href="http://weigou.baidu.com/item?id={%$item.id|escape:'url'%}" target="_blank" title="{%$product_name|escape:'html'%}" class="ecl-weigou-product-link"></a>{%/if%}
                            </div>
                            {%/foreach%}
                        </div>
                    {%else%}
...
<div class="ecl-weigou-nav-buy OP_LOG_BTN" data-click="{fm:'beha'}">
    {%if $tplData.need_transfer%}
    <div>
        <a id="ecl-weigou-nav-buy-transfer" href="http://weigou.baidu.com/search?q={%$extData.OriginQuery|escape:'url'%}&fr=zhixin" target="_blank">查看更多关于"<span class="query-word">{%$extData.OriginQuery|escape:'html'%}</span>"的商品<span class="ecl-weigou-font-songti">&gt;&gt;</span></a>
    </div>
    {%/if%}
...
<script>
    A.init(function(){
        ...
        baidu.dom.ready(function() {
            ...
            loadScript('http://s1.bdstatic.com/r/www/cache/biz/ecom/weigou/weigou_filter-1.4.js');
        });
    });
</script>
{%/if%}
