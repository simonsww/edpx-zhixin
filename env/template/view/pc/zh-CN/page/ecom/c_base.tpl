...
{%function fe_fn_c_title title='' url='' token='' backEndEncryption='' highlightParam=0 showgw=''%}{%if $title===''%}{%$title=$tplData.title%}{%/if%}{%if $url===''%}{%$url=$tplData.url%}{%/if%}{%if $showgw===''%}{%$showgw=$tplData.showgw%}{%/if%}{%if $showgw%}{%$titleLimitlen=56%}{%else%}{%$titleLimitlen=62%}{%/if%}<a href="{%$url%}" target="_blank">{%$title|limitlen:$titleLimitlen|highlight:{%$highlightParam%}%}</a>{%if $showgw%}{%fe_fn_c_showgw%}{%/if%}{%/function%}
...
{%function fe_fn_c_title_soucang title='' url='' token='' backEndEncryption=''%}{%/function%}

{%function fe_fn_c_title_suffix title='' url='' token='' backEndEncryption=''%}</h3>{%/function%}
