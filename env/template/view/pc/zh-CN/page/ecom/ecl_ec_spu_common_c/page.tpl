...
{%assign var="desc" value=['xinpinshangshi'=>'温暖新机上市', 'renqirexiao'=>'火爆热销机型', 'dijiayouxuan'=>'低至五折起', 'jingxuantehui'=>'最高直降1000元']%}
{%assign var="tabs" value=[]%}
{%foreach $keys as $key%}
    {%foreach from=$tplData.tabs key=index item=tab%}
		{%if $key==$tab.internal_name%}
			{%$tabs[$key]=$tab%}
			{%$tabs[$key]['desc']=$desc[$key]%}
		{%/if%}
    {%/foreach%}
{%/foreach%}
