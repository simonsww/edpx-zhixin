...
{%$tplData.rightFieldRowCount=intval(!!$tplData.concern)+intval(!!$tplData.field0)+intval(!!$tplData.field1)+intval($tplData.field2||$tplData.field3||$tplData.field5)+intval(!!$tplData.link)+intval(!!$tplData.other)+intval(!!$tplData.blog)+intval(!!$tplData.lemmatype)+intval(!!$tplData.showurl)%}
{%*
	参数说明
...
<a href="{%fe_fn_c_title_url url="{%$tplData.url%}" token="baike"%}" target="_blank">{%$tplData.title|highlight:4%}</a>
<span class="tsuf tsuf-op" data="{title : '{%$tplData.title|escape:'javascript'%}', link : '{%fe_fn_c_title_url url="{%$tplData.url%}" token="baike"%}'}"></span>
</h3>
{%/block%}
{%block name='content'%}
    <style data-merge>
    ...
    .op_baikeperson_concern{height:22px; line-height:1.69em; margin-bottom:2px; position:relative;padding-left:56px;}
    .op_baikeperson_concerntext{position:absolute;height:22px;width:40px; padding:0 8px;left:0; top:0px;background:#3288ff; color:#fff;}
    .op_baikeperson_concernright{display:inline-block; height:20px;border-top:1px solid #e6e6e6;border-bottom:1px solid #e6e6e6;background:#f7f7f7;}
    .op_baikeperson_concernright a{display:inline-block;height:20px; line-height:1.54em;float:left;  border-right:2px solid #3288ff; padding:0 8px; text-decoration:none; color:#333;}
    .op-baikeperson-showurl{margin-bottom:1px;}
    </style>    
	<div class="c-row op_baikeperson_container">
        {%if !$tplData.hidePic%}
        <div class="c-span6 c-gap-bottom-small">
            <a href="{%$tplData.url%}" target="_blank"><img style="display:block;" class="c-img c-img6" {%if $tplData.tip[0]['isSquare']!="true" && $tplData.tip[0]['horizontal']!="true"%}height="{%fe_fn_c_get_img_height rate=1.157%}"{%/if%} src="{%$tplData.realpicsrc%}"></a>
        </div>
        {%/if%}
        <div class="{%if !$tplData.hidePic%}c-span18{%else%}c-span24{%/if%} c-span-last">
            ...
            {%/if%}
            {%if $tplData.field5[0].value%}
            <p>{%if $tplData.field5[0]['name']%}{%$tplData.field5[0]['name']|escape:'html'%}{%/if%}{%$tplData.field5[0]['value']|limitlen:54|highlight:4%}</p>
            {%/if%}
            {%if !$tplData.field5[0].value&&($tplData.field2[0].value||$tplData.field3[0].value)%}
            <p>
            ...
            {%/if%}
            <p class="op-baikeperson-showurl">{%fe_fn_c_showurl%}{%fe_fn_c_likeshare%}</p> 
        </div>
    </div>
{%/block%}
