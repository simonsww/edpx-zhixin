...
class Ak_McClient {
    ...
    protected function _getMcByOrder($index, $aux_mc = false) {
        ...
        if ($aux_mc) {
            if (empty($this->mc_aux_list[$index])) {
                $this->mc_aux_list[$index] = self::_get_raw_mc($this->conf, $this->resources[$index]['children']);
            }
            return $this->mc_aux_list[$index];
        } else {
            if (empty($this->mc_list[$index])) {
                $this->mc_list[$index] = self::_get_raw_mc($this->conf, $this->resources[$index]['children']);
            }
    ...
    public function set($key, $value, $expire = null) {
        return $this->setImp($key, $value, $expire); 
    }
    ...
    protected function setImp($key, $value, $expire = null, $main_key = null, $sub_key = null) {
        if ($sub_key === null) {
            $key = $this->prefix.$key;
        }
        if (is_null($expire)) {
        ...
        case "none": 
        case "self":
            $ret_all = $this->_set($this->_getMcByOrder(0), $key, $value, $expire);
            break;
        case "all":
            $ret_all = true;
            for ($i = 0; $i < $this->zk_conf['idc_num']; ++$i) {
                ...
                $ret_all = $this->_set($mc, $key, $value, $expire) && $ret_all;
            }
            break;
        default:
            self::_warning("conf error, no such idc_type(".($this->idc_type).")");
    ...
    public function setMulti($items) {
        ...
        case "none": 
        case "self":
            $ret_all = $this->_setMulti($this->_getMcByOrder(0), $items, $expire);
            break;
        case "all":
            $ret_all = true;
            for ($i = 0; $i < $this->zk_conf['idc_num']; ++$i) {
                ...
                $ret_all = $this->_setMulti($mc, $items, $expire) && $ret_all;
            }
            break;
        default:
            self::_warning("conf error, no such idc_type(".($this->idc_type).")");
        ...
        if ($this->conf['set_call_delete_delay']) {
            ...
            foreach ($items as $key => $value) {
                $ret_all = $this->call_acm($key, $key) && $ret_all;
            }
        }
        return $ret_all;
    }
    ...
    protected function _call_acm($key, $main_key, $sub_key) {
        if ($sub_key !== null) {
            return $this->call_acm($key, $main_key, $sub_key);
        } else {
            return $this->call_acm($key, $key);
        }
    }
    ...
    public function setChild($main_key, $sub_key, $value, $expire = null) {
        $key = $this->_getSubKey($main_key, $sub_key);
        return $this->setImp($key, $value, $expire, $this->prefix.$main_key, $sub_key);
    }
    ...
    public function getServersByKey($key) {
        if ($this->mc_list_aux == null) {
            ...
            foreach ($server_lists as $server_list) {
                $this->span_idc->registerResource(self::CACHE_AUX_RESOURCE, $server_list['idc'], $server_list);
            }
        }
        $res = array();
        switch ($this->idc_type) {
