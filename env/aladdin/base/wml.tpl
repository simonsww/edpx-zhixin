...
{%if $tplData.url||$tplData.titleurl%}<a href="{%if $tplData.url%}{%fe_fn_get_url url={%$tplData.url%} type='title'%}{%elseif $tplData.titleurl%}{%fe_fn_get_url url={%$tplData.titleurl%} type='title'%}{%/if%}">{%/if%}{%$alaData.iteration%}&#160;{%$tplData.title|strip_tags:false|wordtruncate:$pageData.titlen:"UTF-8":"..":"TAG":"em-font"%}{%if $tplData.url||$tplData.titleurl%}</a>{%/if%}{%if $fe_var_waplogo%}&#160;<img alt="手机版" src="http://m.baidu.com/static/search/key.gif"/>{%/if%}<br />
{%/if%}
{%/block%}
